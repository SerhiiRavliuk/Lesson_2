...
class HeartbeatMonitoringLogger:
    ...
    def __log_based_on_timedelta(self, start_time: datetime, end_time: datetime):
        if (end_time - start_time) == timedelta(seconds=32):
            self.__write_warning_logs(end_time)
        elif (end_time - start_time) >= timedelta(seconds=33):
            self.__write_error_logs(end_time)
    ...
    def __write_warning_logs(self, end_time: datetime) -> None:
        logger.warning(
            f"Warning Log: Серцева частота виявила часовий інтервал між 31 і 33 секундами. У цьому випадку: {end_time.strftime('%H:%M:%S')}"
        )
    def __write_error_logs(self, end_time: datetime) -> None:
        logger.error(
            f"Error Log: Серцева частота виявила часовий інтервал, що перевищує 33 секунди. У цьому випадку: {end_time.strftime('%H:%M:%S')}"
        )
